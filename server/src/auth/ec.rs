...
pub struct EcAuthServer {
    verifier: Arc<Mutex<EcVerifier>>,
    registrations: Arc<Mutex<HashMap<String, Registration<EcPoint>>>>,
    authentication_states: Arc<Mutex<HashMap<String, AuthenticationState<EcPoint>>>>,
}
...
impl AuthServer for EcAuthServer {
    async fn register(
        ...
        let request: RegisterRequest<EcPoint> = request.from_proto(&mut verifier.ctx)?;
    ...
    async fn create_authentication_challenge(
        ...
        let mut verifier = self
            ...
            .lock()
            .map_err(|_| Status::internal("Lock error"))?;
        let request: AuthenticationChallengeRequest<EcPoint> =
